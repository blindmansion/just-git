...
`just-git/repo` provides a programmatic API for working with git repositories — reading commits, diffing trees, creating objects, and merging — without going through command execution.
...
const git = createGit({
  hooks: {
    postCommit: async ({ repo, hash, parents }) => {
      ...
      console.log("changed:", files.map((f) => f.path));
    },
  },
});
...
Both return `{ treeHash, clean, conflicts, messages }`. Operates purely on the object store — no filesystem or worktree needed.
