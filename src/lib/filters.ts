...
// ── Cached resolution for worktree seams ────────────────────────────

/**
 * Per-context, per-operation memo of {@link bindFilters}. A single command
 * (checkout, add, status) touches many files through the same `ctx`; caching
 * the bound form means the capability context and each `.gitattributes` file
 * are built/read once per command rather than once per file.
 */
const filtersCache = new WeakMap<
	GitContext,
	Map<GitOperation, Promise<BoundFilters | undefined>>
>();

/**
 * Resolve {@link BoundFilters} for a worktree seam, memoized per `ctx`. The
 * synchronous capability check keeps the no-filter path allocation-free; only
 * when `filters` is present do we build (and cache) the bound form.
 */
export function resolveFilters(
	ctx: GitContext,
	operation: GitOperation,
): Promise<BoundFilters | undefined> {
	if (!ctx.capabilities?.filters) return Promise.resolve(undefined);

	let byOp = filtersCache.get(ctx);
	if (!byOp) {
		byOp = new Map();
		filtersCache.set(ctx, byOp);
	}
	let pending = byOp.get(operation);
	if (!pending) {
		pending = bindFilters(ctx, operation);
