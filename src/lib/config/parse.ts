// Pure config text engine: parse / serialize / format, format-preserving raw
// edits, dotted-key splitting, and the {@link ConfigData} value type. A
// dependency-free leaf — no filesystem, no `GitContext`, no other `lib` runtime
// imports — so a consumer that only needs (say) `configBool` or `parseConfig`
// doesn't transitively load the store, the capability context, or attributes.
 ...
 */
export function parseDottedKey(dottedKey: string): {
	section: string;
	key: string;
} {
...
export function tryParseDottedKey(dottedKey: string): { section: string; key: string } | null {
	try {
...
/**
 * Whether a config handle is filesystem-backed (has an `fs` and a `gitDir`),
 * versus a bare store-backed {@link GitRepo}. A dependency-free structural
 * guard so callers that only branch on backing (e.g. diff-driver resolution)
 * don't transitively load the config store or capability machinery.
 */
