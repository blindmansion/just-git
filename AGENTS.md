...
| `git log`         | `commands/log.ts`         | `--oneline`, `-n <count>`, `--all`, `--reverse`, `A..B`, `A...B`, `-- <path>` (pathspec globs), `--author=<pattern>`, `--grep=<pattern>`, `--since`/`--after`/`--until`/`--before`, `--decorate`, `--graph`, `-p`/`--patch`, `--stat`, `--name-status`, `--name-only`, `--shortstat`, `--numstat`. Accepts `<ref>` starting points (e.g. `git log main`)            |
