...
**Maintenance — `gcRepo` / `repackRepo`** (`storage/gc.ts`, free functions over `(repo, driver, repoId, options?, extraTips?)`):
- `gcRepo` prunes unreachable objects (reachability sweep, with a ref-snapshot abort guard against concurrent pushes). `GcOptions.compact` additionally delta-compresses reachable history in the same pass (`window`/`depth` knobs, defaults 10/50 — same as the filesystem `git gc`). Compaction runs **before** the deletion guard since rewriting an encoding never changes a hash, so it is safe even if deletion aborts.
- `repackRepo` is the "compress only" counterpart (compaction without the deletion phase). `GcResult`/`RepackResult` surface `deltified`, `bytesBefore`, `bytesAfter`.
...
- The server exposes `server.gc(repoId, { compact, window, depth, dryRun })` and `server.repack(repoId, options?)`; both fold fork ref tips into `extraTips` when run on a root repo. Triggering policy (cadence, commit-count, byte-pressure via `repoByteSize`) is left to the consumer.
...
- `server.gc(repoId, options?)` — prune unreachable objects (`GcOptions`: `dryRun`, plus `compact`/`window`/`depth` to also delta-compress reachable history). `server.repack(repoId, options?)` — compress-only counterpart (no pruning). Both include fork ref tips when run on a root repo. See [Maintenance](#storage-backends-storage).
