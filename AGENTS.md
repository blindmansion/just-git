...
- `signing` — `{ signer?: Signer; verifier?: Verifier }`. Pluggable commit/tag signing and verification as **injected host capabilities** (same seam as `mergeDriver`). `signer` (write side) is `(payload, opts?) => string | Promise<string>` and returns an armored block (`-----BEGIN PGP/SSH SIGNATURE-----`); the first cut only ever passes `payload` (the host signer picks its own key). `verifier` (read side) is `(payload, signature) => VerificationResult` whose `status` mirrors git's `%G?` codes (`good`/`bad`/`unknown`/`expired`/`revoked`/`cannot-check`); it is frequently pure-TS and sandbox-safe, so it can be supplied even when signing cannot. Both halves are independent and optional, and flow through `GitExtensions` onto `GitContext` (`ctx.signer`/`ctx.verifier`). **Policy vs. mechanism:** the signer/verifier are mechanism only — _whether_ to sign/verify is gated by git config (`commit.gpgsign`, `tag.gpgsign`, `tag.forcesignannotated`, `merge.verifysignatures`, `pull.verifysignatures`) read through `getConfigValue` plus per-command flags (`commit -S`/`--no-gpg-sign`, `tag -s`/`--no-sign`, `merge`/`pull --verify-signatures`). just-git treats all key/program/trust config (`user.signingkey`, `gpg.program`, etc.) as opaque host concerns inside the closure. When policy requires a signature but no signer is injected, the operation **errors** (`gpg failed to sign the data`) rather than silently emitting an unsigned object — so locking `commit.gpgsign=true` in a sandbox is enforced. Signatures round-trip on the object model: `Commit.gpgsig` (a `gpgsig` header) and `Tag.gpgsig` (appended after the message body). `commitSigningPayload(commit)` / `tagSigningPayload(tag)` (exported from `just-git` and `just-git/repo`) produce the exact signed bytes; `verifyCommit(repo, ref, verifier?)` / `verifyTag(repo, ref, verifier?)` (from `just-git/repo`) return a `VerificationResult | null` (`null` = unsigned). `configBool()` (in `lib/config.ts`) normalizes git boolean config. SDK writers (`createCommit`/`buildCommit`/`commit`/`createAnnotatedTag`) take `sign?: boolean` (policy, three-state) and `signer?: Signer` (mechanism); the shared `writeCommitAndAdvance` chokepoint covers `merge`/`rebase`/`cherry-pick`/`revert`/`pull`. Stash and merge-base "virtual" commits are intentionally never signed.
...
`Storage` (thin raw key-value CRUD) is the user-facing abstraction. Drivers implement raw object/ref I/O and an `atomicRefUpdate` primitive; the internal adapter handles object hashing, pack ingestion, symref resolution, and CAS semantics. `createRepoStore(driver?)` wraps a driver with this git-aware adapter; the `driver` is optional and defaults to `MemoryStorage` (mirroring `createServer`). All `Storage` methods use `MaybeAsync<T>` (`T | Promise<T>`) return types so sync (SQLite) and async (Pg) drivers both work. Three optional fork methods (`forkRepo?`, `getForkParent?`, `listForks?`) enable fork semantics — all built-in backends implement them. The adapter handles object fallback (fork reads from parent) and fork-of-fork flattening.

**Object encoding at rest (delta / zlib compaction).** Each object row carries an `encoding` (`raw` | `raw-zlib` | `delta` | `delta-zlib`) and a nullable `base_hash` (two columns on `git_objects`; an extra field in `MemoryStorage`'s map). The ingest hot path (`putObject`/`putObjects`) always writes `raw`. `getObject`/`getObjects` return the **stored representation** (`StoredObject` = `{ type, encoding, baseHash?, content }`); the adapter reconstructs the raw body — inflating `*-zlib` and recursively `applyDelta`-ing `delta*` rows against their base (own-partition first, then parent for forks; bounded by a delta-chain cap). The object hash is always of the reconstructed raw body, so addressing/integrity are independent of encoding. `putDeltaObjects(repoId, rows)` is the compaction writer (insert-or-**replace**, one atomic batch so a delta is never visible before its base). Optional `repoByteSize?(repoId)` reports the summed `content` length for disk-pressure-driven maintenance. `createRepoStore` is the only git-aware layer; backends just persist the columns.

**Maintenance — `RepoStore.gc(repoId, options?)`** (fork-safe, the only public entry point):

- `gc` prunes unreachable objects (reachability sweep, with a ref-snapshot abort guard against concurrent pushes). `GcOptions.compact` additionally delta-compresses reachable history in the same pass (`window`/`depth` knobs, defaults 10/50 — same as the filesystem `git gc`). Compaction runs **before** the deletion guard since rewriting an encoding never changes a hash, so it is safe even if deletion aborts. `GcOptions.prune` (default `true`) toggles the deletion phase — `gc({ compact: true, prune: false })` is the "compress only" mode (compact live history without dropping orphaned objects). `GcOptions.compress` (default `true`) toggles zlib — with `compress: false`, objects are stored `raw`/`delta` (delta-encoding still applies; only the zlib CPU is skipped, e.g. for browser/`fflate` clients). Incompressible objects never grow even when `true` (the smaller of raw vs zlib wins). `GcResult` surfaces `deleted`, `retained`, `deltified`, `bytesBefore`, `bytesAfter`.
- Compaction restricts delta-base candidates to the repo's **own partition** (`listObjectHashes` never includes a parent), which enforces the fork-safe rule that a delta's base outlives it. Reachable content is read through the adapter, so existing delta rows decode to raw first — re-compaction re-deltas from raw rather than stacking delta-of-delta.
- **Fork safety** is built in: `repoStore.gc` collects the ref tips of all forks of a root repo into the reachability roots so a fork's shared bases in the root partition are never pruned. (The underlying `gcRepo` in `store/gc.ts` is an internal primitive — not exported; go through the manager.) `server.gc` delegates to `repoStore.gc`. Triggering policy (cadence, commit-count, byte-pressure via `repoByteSize`) is left to the consumer.

**`just-git/store` vs `just-git/server` exports.** The compaction machinery (`createRepoStore`, `RepoStore`, `StoredObject`, `ObjectEncoding`, `DeltaObjectRow`, `GcOptions`/`GcResult`) is exported from `just-git/store` — the fresh, client-facing contract. `just-git/server` re-exports only the legacy baseline for back-compat: the backends, `Storage`/`RefOps`/`RawRefEntry`/`MaybeAsync`/`CreateRepoOptions`, and `GcOptions`/`GcResult`. New storage types (`createRepoStore`, `RepoStore`, `StoredObject`, `ObjectEncoding`, `DeltaObjectRow`) are intentionally not re-exported from `server`.

| Backend                      | File                              | Construction                                  | Database interface                                                        |
| ---------------------------- | --------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------- |
| `MemoryStorage`              | `store/memory-storage.ts`         | `new MemoryStorage()`                         | None                                                                      |
| `BunSqliteStorage`           | `store/bun-sqlite-storage.ts`     | `new BunSqliteStorage(db)`                    | `BunSqliteDatabase` (native `bun:sqlite`)                                 |
| `BetterSqlite3Storage`       | `store/better-sqlite3-storage.ts` | `new BetterSqlite3Storage(db)`                | `BetterSqlite3Database` (native `better-sqlite3`)                         |
| `PgStorage`                  | `store/pg-storage.ts`             | `await PgStorage.create(pool)`                | `PgPool` (duck-typed, matches `pg.Pool`)                                  |
| `DurableObjectSqliteStorage` | `store/do-sqlite-storage.ts`      | `new DurableObjectSqliteStorage(ctx.storage)` | `DurableObjectStorageSql` (duck-typed, matches CF `DurableObjectStorage`) |
...
- `server.gc(repoId, options?)` — prune unreachable objects (`GcOptions`: `dryRun`, `prune`, `compress`, plus `compact`/`window`/`depth` to also delta-compress reachable history). Delegates to `repoStore.gc`, so fork ref tips are included when run on a root repo. For compress-only, use `server.gc(id, { compact: true, prune: false })`. See [Maintenance](#storage-backends-store).
- `onRefUpdate?: (event) => void` (server config) — low-level, fire-and-forget notification fired after **any** successful ref change: pushes (HTTP/SSH), `server.commit`, and `server.updateRefs`. Distinct from the `ServerHooks` auth/policy channel — it cannot reject, carries no policy semantics, and fires for in-process writes too (mirroring git's `reference-transaction`/`post-update`). `RefChangeEvent` carries `{ repo, repoId, updates, source: "push"|"commit"|"update-refs", auth? }` (`auth` present only for `push`). Intended for push/event-driven sync (notify clients instead of polling).
...
- `listRemoteRefs(url, ctx?)`, `cloneInto(repo, url, ctx?)`, `fetch(repo, remote, ctx?)`, `push(repo, remote, ctx?)` — client-side network operations over a `GitRepo` + `TransportContext` (thin wrappers over `lib/transport/`). Server-free; work against both filesystem-backed and storage-backed repos. `TransportContext` is `{ fetchFn?, credentials?, networkPolicy?, onProgress?, resolveRemote? }` (`credentials` accepts a static `HttpAuth` or a `CredentialProvider`). Refspecs default to the conventional values (`cloneInto`/`fetch` need no refspec; `push` takes `branch`/`src`/`dst`/`force` sugar with a raw `refspecs` escape hatch — mutually exclusive). `fetch` takes a `name` (default `"origin"`) that drives the default tracking namespace (`+refs/heads/*:refs/remotes/<name>/*`) and the `preFetch`/`postFetch` `remote` payload. `push` returns structured per-ref status (`ok`/`up-to-date`/`rejected-non-fast-forward`/`rejected-fetch-first`/`rejected-already-exists`/`rejected-hook`/`error`) plus `forced`/`deleted` flags (`result.ok` is true iff every ref is `ok`/`up-to-date`). `cloneInto` stops at objects + refs + HEAD (no worktree); it writes both local `refs/heads/*` and `refs/remotes/<remote>/*` tracking refs (configurable `remote`, default `"origin"`; surfaced as `trackingRefs`), so ahead/behind vs the remote is answerable without an initial fetch. Hooks fire via `repo.hooks` (`preClone`/`postClone`, `preFetch`/`postFetch`, `prePush`/`postPush`) — no separate hook channel on the context.
- `pull(repo, options, ctx?)` (in `operations.ts`, alongside `merge`/`rebase`) — the programmatic counterpart to `git pull` / `git pull --rebase`. Fetches a remote branch (into `refs/remotes/<remote>/*`) then integrates it into a local branch (`strategy: "merge" | "rebase"`, default merge; `branch` defaults to the current HEAD branch; `remoteBranch` defaults to `branch`). Returns `{ fetched, integration }` where `integration` is the `MergeResult`/`RebaseResult` — conflicts are returned as data (no in-progress state persisted), carrying the `ours`/`theirs` (merge) or `continuation` (rebase) handles to resolve via `merge`/`rebase` directly. Higher-level publish/retry policy (the equivalent of a `sync` loop) is deliberately left to the caller. `merge`'s `author` is optional (required only when a merge commit is actually created), so an integrate-only caller can omit it for fast-forward/up-to-date outcomes.
