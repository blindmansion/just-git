...
describe("interop: config cross-reading", () => {
	...
	test("real git sets config, just-git reads it", async () => {
		await git.execAsync(["config", "user.name", "Real Author"]);
		await git.execAsync(["config", "user.email", "real@author.com"]);
		await git.execAsync(["config", "core.autocrlf", "false"]);
		await git.execAsync(["config", "custom.mykey", "myvalue"]);
	...
	test("just-git sets config, real git reads it", async () => {
		...
		const r1 = await git.execAsync(["config", "--get", "jg.testkey"]);
		expect(r1.exitCode).toBe(0);
		expect(r1.stdout.trim()).toBe("testvalue");

		const r2 = await git.execAsync(["config", "--get", "jg.number"]);
	...
	test("real git config --list includes just-git entries", async () => {
		const r = await git.execAsync(["config", "--list", "--local"]);
		expect(r.exitCode).toBe(0);
		expect(r.stdout).toContain("jg.testkey=testvalue");
	});
});
...
describe("interop: config file format preservation", () => {
	...
	test("raw config readable after real git writes subsections", async () => {
		await git.execAsync(["config", "merge.ff", "only"]);
		await git.execAsync(["config", "branch.main.remote", "origin"]);
		await git.execAsync(["config", "branch.main.merge", "refs/heads/main"]);
	...
	test("just-git writes don't corrupt real git's sections", async () => {
		...
		const r1 = await git.execAsync(["config", "--get", "merge.ff"]);
		expect(r1.exitCode).toBe(0);

		const r2 = await git.execAsync(["config", "--get", "branch.main.remote"]);
		expect(r2.exitCode).toBe(0);

		const r3 = await git.execAsync(["config", "--get", "alias.co"]);
		expect(r3.exitCode).toBe(0);
		expect(r3.stdout.trim()).toBe("checkout");
	});

	test("config survives multiple interleaved writes", async () => {
		await git.execAsync(["config", "real.key1", "val1"]);
		const b1 = justBash(sandbox);
		await jg(b1, "git config set jg.key1 jval1");
		await git.execAsync(["config", "real.key2", "val2"]);
		const b2 = justBash(sandbox);
		await jg(b2, "git config set jg.key2 jval2");
		await git.execAsync(["config", "real.key3", "val3"]);

		const r1 = await git.execAsync(["config", "--get", "real.key1"]);
		expect(r1.stdout.trim()).toBe("val1");
		const r2 = await git.execAsync(["config", "--get", "jg.key1"]);
		expect(r2.stdout.trim()).toBe("jval1");
		const r3 = await git.execAsync(["config", "--get", "real.key3"]);
...
describe("interop: config unset", () => {
	...
	beforeAll(async () => {
		...
		git = realGitIn(sandbox);
		await git.execAsync(["init"]);
		await git.execAsync(["config", "test.key1", "value1"]);
		await git.execAsync(["config", "test.key2", "value2"]);
	});
	afterAll(() => removeSandbox(sandbox));
	...
	test("just-git unsets config, real git confirms", async () => {
		...
		await expectGitCommandError(git.execAsync(["config", "--get", "test.key1"]));

		const check2 = await git.execAsync(["config", "--get", "test.key2"]);
		expect(check2.stdout.trim()).toBe("value2");
	});

	test("real git unsets config, just-git confirms", async () => {
		await git.execAsync(["config", "--unset", "test.key2"]);
...
describe("interop: config behavioral effects", () => {
	test("real git inits with custom branch name, just-git sees it", async () => {
		...
		try {
			await git.execAsync(["init", "--initial-branch=develop"]);
			writeToSandbox(sandbox, "x.txt", "x\n");
			await git.execAsync(["add", "."]);
			await git.execAsync(["commit", "-m", "on develop"]);
...
describe("interop: subsection config (remotes, branches)", () => {
	...
	beforeAll(async () => {
		...
		writeToSandbox(sandbox, "x.txt", "x\n");
		await git.execAsync(["add", "."]);
		await git.execAsync(["commit", "-m", "init"]);
	});
	afterAll(() => removeSandbox(sandbox));

	test("real git remote add, just-git reads", async () => {
		await git.execAsync(["remote", "add", "origin", "https://github.com/test/repo.git"]);
	...
	test("just-git remote add, real git reads", async () => {
		...
		const r = await git.execAsync(["remote", "-v"]);
		expect(r.exitCode).toBe(0);
	...
	test("just-git sets branch tracking, real git reads", async () => {
		...
		const r = await git.execAsync(["config", "--get", "branch.main.rebase"]);
		expect(r.exitCode).toBe(0);
...
describe("interop: edge-case config values", () => {
	...
	test("config value with spaces", async () => {
		await git.execAsync(["config", "user.name", "First Middle Last"]);
		const b = justBash(sandbox);
	...
	test("config value with special characters", async () => {
		await git.execAsync(["config", "test.url", "https://example.com/path?q=1&b=2"]);
		const b = justBash(sandbox);
	...
	test("config value with equals sign", async () => {
		await git.execAsync(["config", "test.expr", "a=b=c"]);
		const b = justBash(sandbox);
	...
	test("boolean config values", async () => {
		await git.execAsync(["config", "core.bare", "false"]);
		await git.execAsync(["config", "core.ignorecase", "true"]);
...
describe("interop: quoted values and escapes", () => {
	...
	test("value with hash character (real git quotes it)", async () => {
		await git.execAsync(["config", "test.hash", "value # with hash"]);
	...
	test("value with semicolon character", async () => {
		await git.execAsync(["config", "test.semi", "value ; with semi"]);
	...
	test("value with leading whitespace (real git quotes it)", async () => {
		await git.execAsync(["config", "test.leading", "  leading spaces"]);
	...
	test("value with trailing whitespace (real git quotes it)", async () => {
		await git.execAsync(["config", "test.trailing", "trailing spaces  "]);
	...
	test("value with backslash (real git escapes it)", async () => {
		await git.execAsync(["config", "test.bslash", "C:\\Users\\me"]);
	...
	test("value with double quotes (real git escapes them)", async () => {
		await git.execAsync(["config", "test.dquote", 'say "hello"']);
	...
	test("hand-crafted config with inline comment", async () => {
		...
		const rr1 = await git.execAsync(["config", "--get", "manual.key1"]);
	...
	test("hand-crafted config with continuation line", async () => {
		...
		const rr = await git.execAsync(["config", "--get", "multi.line"]);
	...
	test("hand-crafted config with escape sequences", async () => {
		...
		const rr1 = await git.execAsync(["config", "--get", "esc.tabs"]);
		expect(rr1.stdout.trim()).toBe("col1\tcol2");

		const rr3 = await git.execAsync(["config", "--get", "esc.backslash"]);
...
describe("interop: format-preserving writes", () => {
	...
	test("just-git set preserves real git's existing values", async () => {
		await git.execAsync(["config", "core.editor", "vim"]);
		await git.execAsync(["config", "myapp.author", "Real Author"]);

		const b = justBash(sandbox);
		await jg(b, "git config set myapp.version 2");

		const rEditor = await git.execAsync(["config", "--get", "core.editor"]);
		expect(rEditor.stdout.trim()).toBe("vim");

		const rAuthor = await git.execAsync(["config", "--get", "myapp.author"]);
		expect(rAuthor.stdout.trim()).toBe("Real Author");

		const rVersion = await git.execAsync(["config", "--get", "myapp.version"]);
		expect(rVersion.stdout.trim()).toBe("2");
	});

	test("just-git unset preserves other entries", async () => {
		await git.execAsync(["config", "test.keep", "keepme"]);
		await git.execAsync(["config", "test.remove", "removeme"]);
		...
		const rKeep = await git.execAsync(["config", "--get", "test.keep"]);
		expect(rKeep.stdout.trim()).toBe("keepme");

		await expectGitCommandError(git.execAsync(["config", "--get", "test.remove"]));
	});
	...
	test("just-git write produces values real git can read", async () => {
		...
		const r = await git.execAsync(["config", "--get", "test.bslash"]);
		expect(r.exitCode).toBe(0);
...
describe("interop: multi-value keys", () => {
	...
	test("real git --add creates multi-value, just-git --get-all reads them", async () => {
		await git.execAsync(["config", "multi.key", "val1"]);
		await git.execAsync(["config", "--add", "multi.key", "val2"]);
		await git.execAsync(["config", "--add", "multi.key", "val3"]);
	...
	test("real git --get-all agrees with just-git --get-all", async () => {
		const rReal = await git.execAsync(["config", "--get-all", "multi.key"]);
	...
	test("just-git --add appends, real git --get-all sees it", async () => {
		...
		const r = await git.execAsync(["config", "--get-all", "multi.key"]);
		expect(r.exitCode).toBe(0);
		expect(r.stdout).toBe("val1\nval2\nval3\nval4\n");
	});
	...
	test("real git --list agrees on multi-value lines", async () => {
		const rReal = await git.execAsync(["config", "--list", "--local"]);
	...
	test("multi-value remote.origin.fetch refspecs", async () => {
		await git.execAsync(["remote", "add", "myremote", "https://example.com/repo.git"]);
		await git.execAsync(["config", "--add", "remote.myremote.fetch", "+refs/tags/*:refs/tags/*"]);
		...
		const rReal = await git.execAsync(["config", "--get-all", "remote.myremote.fetch"]);
	...
	test("just-git addConfigValueRaw produces values real git can read", async () => {
		...
		const r = await git.execAsync(["config", "--get-all", "jgmulti.val"]);
		expect(r.exitCode).toBe(0);
