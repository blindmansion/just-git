...
| `log`                             | `--oneline`, `-n`, `--all`, `--reverse`, `--decorate`, `--graph`, `--first-parent`, `--format`/`--pretty`, `-p`/`--patch`, `--stat`, `--name-status`, `--name-only`, `--shortstat`, `--numstat`, `A..B`, `A...B`, `-- <path>`, `--author=`, `--grep=`, `--since`/`--after`, `--until`/`--before` |
